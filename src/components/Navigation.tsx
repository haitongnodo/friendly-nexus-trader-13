...
const Navigation = () => {
  ...
  const AppSidebar = () => {
    return (
      <Sidebar className="w-[240px] border-r border-border-subtle bg-[rgba(20,21,26,0.9)] backdrop-blur-sm">
        <SidebarHeader className="p-6">
          <Link to="/" className="flex items-center">
            <svg width="93" height="32" viewBox="0 0 93 40" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <SidebarMenuItem key={item.path}>
                ...
                >
                  <Link 
                    to={item.path}
                    className={cn(
                      "flex h-11 items-center gap-3 px-4 rounded-md transition-all duration-200",
        ...
        <SidebarFooter className="p-6">
          {isWalletConnected ? (
            ...
            <button
              ...
              className="w-full h-11 orange-gradient px-4 rounded-lg font-medium text-white hover:opacity-90 transition-all"
            >
              Connect Wallet
            </button>
          )}
