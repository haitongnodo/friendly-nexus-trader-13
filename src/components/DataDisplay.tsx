...
export const DataDisplay = ({ type, isLoading }: DataDisplayProps) => {
  ...
  return (
    <motion.div 
      ...
      className="p-6"
    >
      <h2 className="text-xl font-semibold mb-6">🔥 Hot SUI Tokens</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {tokens.map((token: HotToken) => (
          <Card 
            key={token.name}
            className="p-4 bg-[rgba(20,21,26,0.9)] hover:bg-card-gradient transition-all duration-200 rounded-xl border border-border-subtle hover:border-border-strong"
          >
            ...
            <button 
              className="text-primary hover:text-primary-hover text-sm transition-colors mt-4"
